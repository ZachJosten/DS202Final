...
```{r}
primeg %>% ggplot(aes(x=Disposition, fill = ShotType)) + geom_histogram()
```

```{r}
primeg %>% ggplot(aes(x = CritChance, y = CritMultiplier, color = `Damage Type`)) + geom_jitter() + scale_color_discrete(type = finalcol) + facet_wrap(~Class)
```
